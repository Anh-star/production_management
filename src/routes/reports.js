...
const { startProduction, stopProduction, getParetoReport, getDailyReport, getAllReports, getReportById } = require('../controllers/reportController');
const auth = require('../middleware/auth');
const authorize = require('../middleware/roles');
const upload = require('../middleware/upload');

router.get('/', auth, authorize(['Admin', 'Planner', 'QC', 'Operator']), getAllReports);
...
const stopValidation = [
  ...
  body('runtime_min', 'Runtime is required').optional().isInt({ min: 0 }),
  body('downtime_min', 'Downtime is required').optional().isInt({ min: 0 }),
  ];
...
const dailyReportValidation = [
    query('start_date', 'start_date is required').isISO8601(),
    query('end_date', 'end_date is required').isISO8601(),
];
...
router.get('/pareto', auth, authorize(['Admin', 'Planner', 'QC', 'Operator']), paretoValidation, getParetoReport);

router.get('/daily', auth, authorize(['Admin', 'Planner', 'QC', 'Operator']), dailyReportValidation, getDailyReport);

router.get('/:id', auth, authorize(['Admin', 'Planner', 'QC', 'Operator']), getReportById);
